...
First, download automata lite from Amelia's artifactory at https://artifactory.amelia.com/ui/native/amelia-midservers/. Select a file in this format, where `<platform-version-number> **Note:** ` is the same as your platform version.
...
>The asterisk (*) in the file name is the version and date number pattern included in the file name. For example, -vx.x.x-yyyymmdd.

<code-block lang="Bash"> **Note:** $: cd automata-lite-installer/
